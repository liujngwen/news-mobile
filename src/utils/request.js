...
instance.interceptors.request.use(config => {
  ...
  if (err.response && err.response.status === 401) {
    ...
    try {
      // 发刷新token的请求
      // 注意：不能使用instance，已经拥有了一些配置，此时的刷新请求不能使用这些配置
      const { data: { data } } = await axios({
        url: 'http://ttapi.research.itcast.cn/app/v1_0/authorization/',
        methods: 'put',
        headers: {
      ...
      // 更新vuex和本地token，使用mutations中的setUser即可
...
// 调用接口 (url接口地址，method请求方式，data对象传参)
// 返回值是一个promise
